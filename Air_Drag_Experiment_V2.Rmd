...
author: "Aguado, B.; Jorges, B.; López-Moliner, J."
date: "2/10/2019"
output:   pdf_document
editor_options:
...
# #    the variability from the timing data collected in Jörges & López-Moliner (2009) 
...
collapsed <- read.table(file = "All_Data_Air_Drag", header = T)
...
air_drag_sum <- air_drag %>%
  ...
  summarize(sd_timing = sd(rtime_timing),
...
mean_ratio_sd_air_Drag <- air_drag_sum %>%
  group_by(id) %>%
  summarize(max_xerror = mean_cl_normal(ratio_x)$ymax,
